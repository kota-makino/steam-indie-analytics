...
class SteamAPIRateLimiter:
    ...
    def __init__(self, max_requests: int = 200, time_window: int = 300):
        """
        Args:
...
class SteamAPIClient:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            raise ValueError("Steam API キーが設定されていません。STEAM_API_KEY環境変数を設定してください。")
    ...
    async def _make_request(self, url: str, params: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        if not self.session:
            raise RuntimeError("セッションが初期化されていません。withステートメントを使用してください。")
    ...
    async def get_app_reviews(self, app_id: int, num_per_page: int = 100) -> Dict[str, Any]:
        """ゲームのレビューデータを取得
    ...
    async def filter_indie_games(self, apps: List[Dict[str, Any]], sample_size: int = 1000) -> List[int]:
        """インディーゲームを識別・フィルタリング
        ...
        # インディーゲームの識別キーワード
        indie_keywords = [
            "indie", "independent", "small", "solo", "pixel", "retro", 
            "casual", "adventure", "puzzle", "platformer", "roguelike"
        ]
        ...
        logger.info(f"名前ベースフィルタリング: {len(indie_app_ids)}件のインディーゲーム候補を抽出")
        ...
        logger.info(f"詳細情報ベースフィルタリング: {len(confirmed_indie_games)}件のインディーゲームを確認")
    ...
    def _is_indie_game(self, game_data: SteamGameData) -> bool:
        ...
        if game_data.genres:
            ...
            for genre in game_data.genres:
                if any(keyword in genre.get("description", "").lower() for keyword in indie_genres):
                    return True
        ...
        major_publishers = [
            "valve", "activision", "electronic arts", "ubisoft", "bethesda",
...
async def test_steam_api():
    ...
    async with SteamAPIClient() as client:
        ...
        if game_data:
            ...
            print(f"   ジャンル: {[g.get('description') for g in game_data.genres or []]}")
