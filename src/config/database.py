...
async def get_db_session() -> AsyncGenerator[AsyncSession, None]:
    ...
    if AsyncSessionLocal is None:
        raise RuntimeError("Async database functionality is not available. Install asyncpg.")
...
async def main():
    ...
    if sync_result:
        ...
        with get_sync_session() as session:
            result = session.execute(text("SELECT COUNT(*) FROM games WHERE type = 'game'"))
        ...
        if async_result:
            ...
            async with get_db_session() as session:
                result = await session.execute(text("SELECT COUNT(*) FROM games WHERE type = 'game'"))
