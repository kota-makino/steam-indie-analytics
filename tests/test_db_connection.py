...
def test_postgresql_connection():
    ...
    try:
        ...
        print(f"  ✅ PostgreSQLバージョン: {version.split(',')[0]}")
    ...
    try:
        ...
        print(f"  📊 既存テーブル: {tables if tables else '(なし)'}")
...
def test_redis_connection():
    ...
    print(f"接続先: {redis_config['host']}:{redis_config['port']}/{redis_config['db']}")
    ...
    try:
        ...
        if retrieved_value == test_value:
            ...
            print(f"  ❌ Redis読み込みエラー: 期待値={test_value}, 実際={retrieved_value}")
            return False
...
def test_environment_variables():
    ...
    # オプション環境変数のリスト
    optional_vars = [
        'STEAM_API_KEY', 'DEBUG', 'LOG_LEVEL',
        'STEAM_API_RATE_LIMIT', 'API_RETRY_ATTEMPTS'
    ]
    ...
    if not steam_api_key or steam_api_key == 'your_steam_api_key_here':
        ...
        print("     https://steamcommunity.com/dev/apikey でキーを取得してください")
    ...
    if missing_optional:
        print(f"  ⚠️  オプション環境変数が未設定: {missing_optional}")
    
    print("🎉 環境変数設定テスト完了\n")
    return True


def test_steam_api_configuration():
    """Steam API設定テスト"""
...
def main():
    ...
    if all_passed:
        ...
        print("Steam Analytics プロジェクトの基盤は正常に動作しています。")
    else:
