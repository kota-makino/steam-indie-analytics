        ...
        with urllib.request.urlopen(request, timeout=30) as response:
            if response.status == 200:
                ...
                for i, app in enumerate(apps[:5]):
                    print(f"  {i+1}. {app.get('name', 'N/A')} (ID: {app.get('appid', 'N/A')})")
        ...
        with urllib.request.urlopen(request, timeout=30) as response:
            if response.status == 200:
                ...
                if app_data and app_data.get('success'):
                    ...
                    print(f"  パブリッシャー: {game_info.get('publishers', ['N/A'])}")
                    ...
                    if price_info:
                        ...
                        final_price = price_info.get('final', 0) / 100  # セントをドルに変換
...
def test_database_connection_simple():
    ...
    try:
        ...
        print("⚠️  psycopg2が利用できません（正常: requirements.txtから後でインストール）")
